...
$paymentData = null;
if (stripos($asaasId, 'http') === 0) {
    $listResponse = $asaas->findPaymentByInvoiceUrl($asaasId);
    $list = $listResponse['ok'] ? ($listResponse['data']['data'] ?? []) : [];
    $paymentData = $list[0] ?? null;
} elseif (stripos($asaasId, 'pay_') === 0) {
...
if (in_array($status, $paidStatuses, true)) {
    ...
    if (empty($pendenciaRow['paid_at'])) {
        ...
        $payloadUpdate['access_code'] = $accessCode;
        $payloadUpdate['payment_date'] = $dayUseDate;
        if ($studentId) {
            $payloadUpdate['student_id'] = $studentId;
        }
        if ($enrollment !== null) {
            $payloadUpdate['enrollment'] = $enrollment;
        }
...
<!doctype html>
<html lang="pt-BR">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Pagamento confirmado</title></head>
<body style="margin:0;padding:0;background:#EEF2F7;">
<table width="100%" style="background:#EEF2F7;padding:24px;"><tr><td align="center">
<table width="600" style="background:#FFF;border-radius:18px;box-shadow:0 10px 30px rgba(11,16,32,.14);">
...
<div style="font-size:13px;opacity:.9;">Pagamento confirmado</div>
</td></tr>
<tr><td style="padding:28px;">
<div style="font-size:26px;font-weight:800;">Pagamento confirmado ✅</div>
<div style="margin-top:10px;font-size:15px;">Tudo certo! Recebemos o pagamento da diária e o acesso foi <b>liberado automaticamente</b>.</div>
...
<tr><td style="padding:18px 28px;background:#F3F6FB;font-size:12px;color:#556070;text-align:center;">Diárias Village</td></tr>
</table>
</td></tr>
</table>
</body>
...
HTML;
            ...
            if ($secretaria) {
                $replace['{{extra_dados}}'] = '<div style="margin-top:16px;background:#F6F8FC;border:1px solid #E6E9F2;border-radius:14px;padding:16px;">
                  ...
                  <div style="font-size:13px;">CPF: <b>' . htmlspecialchars($pendenciaRow['guardian_cpf'] ?? '', ENT_QUOTES, 'UTF-8') . '</b><br>
