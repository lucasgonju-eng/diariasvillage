...
function adminCreateStudentApiError(array $result, string $fallback): string
{
    $parts = [];
    if (isset($result['status'])) {
        $parts[] = 'HTTP ' . (string) $result['status'];
    }
    if (is_array($result['data'] ?? null)) {
        foreach (['message', 'details', 'hint', 'code'] as $key) {
            $value = trim((string) ($result['data'][$key] ?? ''));
            if ($value !== '') {
                $parts[] = $value;
            }
        }
...
try {
    ...
    if (!($duplicateEnrollment['ok'] ?? false)) {
        $detail = adminCreateStudentApiError($duplicateEnrollment, 'Falha ao validar matrícula existente.');
        error_log('[admin-create-student] duplicate enrollment check failed: ' . $detail);
        Helpers::json(['ok' => false, 'error' => 'Falha ao validar matrícula existente: ' . $detail], 500);
    }
    ...
    if (!($duplicateName['ok'] ?? false)) {
        $detail = adminCreateStudentApiError($duplicateName, 'Falha ao validar aluno existente.');
        error_log('[admin-create-student] duplicate name check failed: ' . $detail);
        Helpers::json(['ok' => false, 'error' => 'Falha ao validar aluno existente: ' . $detail], 500);
    }
    ...
    if (!($guardianEmailResult['ok'] ?? false)) {
        $detail = adminCreateStudentApiError($guardianEmailResult, 'Falha ao validar e-mail do responsável.');
        error_log('[admin-create-student] guardian email check failed: ' . $detail);
        Helpers::json(['ok' => false, 'error' => 'Falha ao validar e-mail do responsável: ' . $detail], 500);
    }
    if (!empty($guardianEmailResult['data'])) {
        Helpers::json(['ok' => false, 'error' => 'Este e-mail já está vinculado a outro cadastro.'], 409);
    }

    $guardianDocumentResult = $client->select(
        'guardians',
        'select=id,student_id,parent_document&parent_document=eq.' . urlencode($guardianDocument) . '&limit=1'
    );
    if (!($guardianDocumentResult['ok'] ?? false)) {
        $detail = adminCreateStudentApiError($guardianDocumentResult, 'Falha ao validar CPF do responsável.');
        error_log('[admin-create-student] guardian document check failed: ' . $detail);
        Helpers::json(['ok' => false, 'error' => 'Falha ao validar CPF do responsável: ' . $detail], 500);
    }
    if (!empty($guardianDocumentResult['data'])) {
        Helpers::json(['ok' => false, 'error' => 'Este CPF já está vinculado a outro cadastro.'], 409);
    }
    ...
    if (!($studentInsert['ok'] ?? false) || empty($studentInsert['data'][0])) {
        $detail = adminCreateStudentApiError($studentInsert, 'Falha ao criar aluno.');
        error_log('[admin-create-student] student insert failed: ' . $detail);
        Helpers::json(['ok' => false, 'error' => 'Falha ao criar aluno: ' . $detail], 500);
    }
    ...
    if (!($guardianInsert['ok'] ?? false) || empty($guardianInsert['data'][0])) {
        $detail = adminCreateStudentApiError($guardianInsert, 'Falha ao criar responsável.');
        error_log('[admin-create-student] guardian insert failed: ' . $detail);
        $client->delete('students', 'id=eq.' . urlencode($studentId));
        Helpers::json([
            'ok' => false,
            'error' => 'Falha ao criar responsável. O cadastro do aluno não foi mantido: ' . $detail,
        ], 500);
    }
    ...
    error_log('[admin-create-student] fatal: ' . $e->getMessage() . ' @ ' . $e->getFile() . ':' . $e->getLine());
    Helpers::json(['ok' => false, 'error' => 'Falha interna ao criar aluno. Detalhe registrado no log.'], 500);
}
